...
static Std_ReturnType Elf_LoadProgram(const Elf32_Ehdr *elf_image, MemoryAddressMapType *memap)
{
	...
	for (i = 0; i < elf_image->e_phnum; i++) {
		 ...
		 * ROM領域のみロードする．
		 */
		Elf32_Addr	p_paddr = phdr->p_paddr;
		if (p_paddr == NULL)
			p_paddr = phdr->p_vaddr;
		ptr = mpu_address_get_rom(p_paddr, phdr->p_filesz);
		if (ptr == NULL) {
			continue;
		}
		err = mpu_get_pointer(CPU_CONFIG_CORE_ID_0, p_paddr, &ptr);
		if (err != STD_E_OK) {
		...
		printf("Elf loading was succeeded:0x%x - 0x%x : %u.%u KB\n", p_paddr, p_paddr + phdr->p_memsz, phdr->p_filesz/1024, phdr->p_filesz % 1024);
